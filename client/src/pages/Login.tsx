...
const Login = () => {
  ...
  const handleSubmit = async (e:React.FormEvent) =>{
    ...
    try {
      ...
      const errorMsg = error.response?.data?.error?.message || "Invalid credentials or user does not exist.";
        ...
        <main className="login-page-container">
          <form onSubmit={handleSubmit} className="login-form">
            ...
            <p className="mt-2 text-sm text-gray-500/90 dark:text-gray-400">
              {state === 'login' ? 'Please enter email and password to access' : 'Please enter your detail to create an account.'}
            </p>
            {/* Username */}
